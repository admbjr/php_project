...
class Destination
{
    public function getAllDestination($dbcon){
     ...
     public function getAllDestinationByUser($dbcon, $userId){
        $sql = "SELECT * FROM destinations WHERE user_id = :user_id";
        $pdo = $dbcon->prepare($sql);
        $pdo->bindParam(':user_id', $userId);
        $pdo->execute();
        $desData = $pdo->fetchAll(PDO::FETCH_OBJ);
        return $desData;
    }
    
	 public function addDestination($userId, $country, $city, $caption, $des_image, $db)
    {
        $sql = "INSERT INTO destinations (user_id, country, city, caption, image) 
              VALUES (:user_id, :country, :city, :caption, :image) ";
        $pdo = $db->prepare($sql);
        $pdo->bindParam(':user_id', $userId);
